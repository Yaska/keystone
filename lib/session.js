...
var whitelistRE;
var keystoneRE = new RegExp('^/' + keystone.get('admin path') + '/?$', 'i');
	...
	if (!req.user || !req.user.canAccessKeystone) {
		...
		if (!checkedAllow) {
			...
			if (allowed) {
				...
				whitelistRE = new RegExp(
					'^/' + keystone.get('admin path') + '/api/(' + regexes + ')(/|\\?|$)'
				);
			}
		}
		if (
			whitelistRE &&
			req.method === 'GET' &&
			whitelistRE.test(req.originalUrl)
		) {
			return next();
		} else {
		...
		var from = keystoneRE.test(req.originalUrl)
			? ''
